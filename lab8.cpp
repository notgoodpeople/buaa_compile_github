...
char key[12][15] = {"int", "main", "return", "const", "if", "else", "while", "break", "continue", "void"};
char keyOut[12][15] = {"Int", "Main", "Return", "Const", "If", "Else", "While", "Break", "Continue", "Void"};
char funcCall[9][15] = {"getint", "putint", "getch", "putch", "putarray", "getarray"};
char funcCallOut[9][15] = {"getint", "putint", "getch", "putch", "putarray", "getarray"};
struct symType
{ //正在分析的词
	string name;
	/*类型：1~10 为关键字：int，main，return，const，if，else, while, break ,continue ,void
...
//不再使用这个函数。在表达式计算，Exp()类函数时 使用该函数报错
void error()
{
	ret = 120;
	printf("\nExp() error");
...
void initFunc()
{
	...
	f_getch.paramsNum = 0;
	FuncMap["getch"] = f_getch;

	//int getarray();
	fprintf(fpout, "declare i32 @getarray(i32*)\n");
    struct FuncItem f_getarray;
	vector<int> Item_getarray;
	vector<int> Dim_getarray;
	Item_getarray.push_back(2);
	Dim_getarray.push_back(1);
    f_getarray.RetType = 1;
    f_getarray.params = Item_getarray;
	f_getarray.paramsDimension = Dim_getarray;
	f_getarray.funcName = "@getarray";
	f_getarray.paramsNum = 1;
    FuncMap["getarray"] = f_getarray;

	//void putint(int);
	fprintf(fpout, "declare void @putint(i32)\n");
	struct FuncItem f_putint;
	vector<int> Item_putint;
	vector<int> Dim_putint;
	Item_putint.push_back(1);
	Dim_putint.push_back(0);
	f_putint.RetType = 0;
	f_putint.params = Item_putint;
	f_putint.paramsDimension = Dim_putint;
	f_putint.funcName = "@putint";
	f_putint.paramsNum = 1;
	FuncMap["putint"] = f_putint;

	//void putch(int);
	fprintf(fpout, "declare void @putch(i32)\n");
	struct FuncItem f_putch;
	vector<int> Item_putch;
	vector<int> Dim_putch;
	Item_putch.push_back(1);
	Dim_putch.push_back(0);
	f_putch.RetType = 0;
	f_putch.params = Item_putch;
	f_putch.paramsDimension = Dim_putch;
	f_putch.funcName = "@putch";
	f_putch.paramsNum = 1;
	FuncMap["putch"] = f_putch;

	//void putarray(int, int[]);
	fprintf(fpout,"declare void @putarray(i32, i32*)\n");
	struct FuncItem f_putarray;
	vector<int> Item_putarray;
	vector<int> Dim_putarray;
	Item_putarray.push_back(1);
	Dim_putarray.push_back(0);
	Item_putarray.push_back(2);
	Dim_putarray.push_back(1);
    f_putarray.RetType = 0;
    f_putarray.params = Item_putarray;
	f_putarray.paramsDimension = Dim_putarray;
	f_putarray.funcName = "@putarray";
	f_putarray.paramsNum = 2;
    FuncMap["putarray"] = f_putarray;
...
int CompUnit()
{
	...
	while ((symNow.type == 1 && (sym[symst + 1].type == 52 ||
								 ...
								 sym[symst + 1].type == 72 || sym[symst+1].type == 55)) ||
		   symNow.type == 4 || symNow.type == 10)
	{
		if((symNow.type == 1 && (sym[symst + 1].type == 52 ||
								 sym[symst + 1].type == 54 || sym[symst + 1].type == 53 ||
								 sym[symst + 1].type == 72)) ||
		    symNow.type == 4){
			GlobalDef = true;
		...
		else{   //void或者sym[symst+1].type == 55
			GlobalDef = false;
		    ret = FuncDef();
			GlobalDef = true;
			if (ret != 0)
...
int FuncDef()
{
	...
	if(symNow.type == 1){
		fprintf(fpout, "i32 ");
		tempFuncItem->RetType = 1;
	}
	else if(symNow.type == 10){
		fprintf(fpout, "void ");
		tempFuncItem->RetType = 0;
	}
	// if (ret != 0)
	// 	return ret;
	symNow = sym[symst++];
	ret = Ident();      //仅检测语法是否合理
	if(symNow.type != 2){
		fprintf(fpout, "@%s",symNow.name.c_str());
	}
	tempFuncItem->funcName = "@"+symNow.name;
	tempFuncItem->tfuncName = symNow.name;
	...
	fprintf(fpout, "(");
	tempFuncItem->paramsNum = 0;
	if(sym[symst].type != 56){
		symNow = sym[symst++];
		FuncFParams();
	}
...
int FuncType()
{
	if (symNow.type != 1 && symNow.type != 10)
	{
...
void FuncFParams(){
	...
	tempFuncItem->paramsNum = paramsNum;
}
void FuncFparam(){
	if(symNow.type != 1){
		printf("error in FuncFparam Int");
		throw "Error";
	}
	symNow = sym[symst++];
	if(symNow.type !=22){
	...
	tempVarItem->dimension = tempDimesion;
	if(tempDimesion == 0 ){
		tempFuncItem->params.push_back(1);
		tempFuncItem->paramsDimension.push_back(0);
		fprintf(fpout,"i32 %%x%d",tempVarItem->registerNum);
	}
	else{
		tempFuncItem->params.push_back(2);
		tempFuncItem->paramsDimension.push_back(tempDimesion);
		fprintf(fpout,"i32* %%x%d",tempVarItem->registerNum);
	}
	int arraySize = 1;
	for (int i = 0; i < tempDimesion; i++)
...
{ //只用于FuncDef
	...
	else if (symNow.type == 22) //只用于FuncDef
	{
...
int Stmt()
{
	if (symNow.type == 3)
	{
		...
		sym[symst].type == 22 ||sym[symst].type == 55||
		sym[symst].type == 21){                //有return Exp
			if(tempFuncItem->RetType == 0){
				printf("error in Stmt RetType error");
				throw "Error";
			}
			symNow = sym[symst++];
			Exp();
	...
	else if (symNow.type == 22 && sym[symst].type == 55)
	{
		FuncCall();
		symNow = sym[symst++];
			if (symNow.type != 54)
...
int UnaryExp()
{
	...
	else if ((symNow.type >= 11 && symNow.type <= 20) || (symNow.type == 22 && sym[symst].type == 55))
	{
		FuncCall();
	}
	else
...
int LVal()
{
	...
	if(varIsParam){
		if((*funcIt).second.paramsDimension[paramSt] != 0){
			isArray = true;
		}
	}
	if(isArray){
	...
	bool savedVarIsParam = varIsParam;
	varIsParam = (*varIt).second.isParam;    //只在下面使用的判断
	if (isArray)
	{
		if(varIsParam){
			fprintf(fpout, "    %%x%d = alloca i32*\n", ++VarMapSt);
...
{
	funcIt = FuncMap.find(symNow.name);
	map<string, struct FuncItem>::iterator savedFuncIt;
	if (funcIt == FuncMap.end())
	{
	...
	else if ((*funcIt).second.RetType == 1)
	{
		...
		if((*funcIt).second.funcName == "@getint"){
			fprintf(fpout, "()\n    call void @putint(i32 %%x%d)\n", tempExpStack->value, (*funcIt).second.funcName.c_str());
	...
	for(int i = 0; i < (*funcIt).second.paramsNum; i++){
		reverseExpStack.push(ExpStack.top());
		ExpStack.pop();
	}
	for (int i = 0; i < (*funcIt).second.paramsNum; i++)
	{
		...
		if((*funcIt).second.params[i] == 1){
			fprintf(fpout, "i32 ", reverseExpStack.top().value);
		}
		else{
		...
		{
			fprintf(fpout, "%d", reverseExpStack.top().value);
		}
		else if (reverseExpStack.top().type == 3)
		{
			fprintf(fpout, "%%x%d", reverseExpStack.top().value);
		}
		else
		{
			printf("error in FuncCall");
