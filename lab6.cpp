...
int GlobalAddExp() {
	...
	if (sym[symst].type == 59|| sym[symst].type == 62) {    //读后面一个词，判断是否正确 
		symNow = sym[symst++];
	}
	while (symNow.type == 59|| symNow.type == 62) {
...
int GlobalMulExp() {
	...
	if (sym[symst].type == 60 || sym[symst].type == 61 || sym[symst].type == 63) {  //读后面一个词，判断是否正确 
		symNow = sym[symst++];
	}
	while (symNow.type == 60 || symNow.type == 61 || symNow.type == 63) {
		struct symType tempSymNow = symNow;
...
int GlobalUnaryExp() {
	...
	if (symNow.type == 59|| symNow.type == 62) {
		int PositiveNum=1;
		if (symNow.type == 59) {        //多余一个+-号，则以此来判断Number的正负 
